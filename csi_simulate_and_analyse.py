...
class CSIConfig:
    ...
    target_attack_variance: float = 0.30071046
...
def make_timestamps(cfg: CSIConfig) -> np.ndarray:
    ...
    times = [
        start_dt + timedelta(milliseconds=i * cfg.sample_interval_ms)
        for i in range(cfg.n_samples)
    ]

    return np.array(
        [t.isoformat(sep=" ") for t in times],
        dtype=object
    )


def scale_series_to_target_variance(
    x: np.ndarray,
    target_var: float
) -> np.ndarray:
    ...
    if x.size < 2:
        raise ValueError(
            "Need at least 2 samples to define sample variance (ddof=1)."
        )
    ...
    if current_var == 0:
        raise ValueError(
            "Current variance is zero; cannot scale."
        )
...
def generate_dataset(cfg: CSIConfig) -> pd.DataFrame:
    ...
    labels = np.array(
        ["normal"] * n_normal + ["attack"] * n_attack,
        dtype=object
    )

    subcarrier_idx = (
        np.arange(cfg.n_samples) % cfg.n_subcarriers
    ).astype(int)
    ...
    amp_normal = (
        scale_series_to_target_variance(
            amp_normal_raw,
            cfg.target_normal_variance
        )
        + cfg.amplitude_mean
    )
    ...
    amp_attack = (
        scale_series_to_target_variance(
            amp_attack_raw,
            cfg.target_attack_variance
        )
        + cfg.amplitude_mean
    )

    amplitude = np.concatenate(
        [amp_normal, amp_attack]
    ).astype(float)

    phase_normal = rng.normal(
        cfg.phase_mean,
        cfg.phase_std_normal,
        n_normal
    )

    phase_attack = rng.normal(
        cfg.phase_mean,
        cfg.phase_std_attack,
        n_attack
    )

    phase = np.concatenate(
        [phase_normal, phase_attack]
    )

    phase = (phase + np.pi) % (2 * np.pi) - np.pi

    rssi_normal = rng.normal(
        cfg.rssi_mean_dbm,
        cfg.rssi_std_normal,
        n_normal
    )

    rssi_attack = rng.normal(
        cfg.rssi_mean_dbm,
        cfg.rssi_std_attack,
        n_attack
    )
...
def analyse_and_report(
    ...
    threshold = (
        cfg.threshold_multiplier * normal_var
    )

    ratio = (
        attack_var / normal_var
        if normal_var > 0
        else np.nan
    )
    ...
    return {
        ...
        "targets_normal_variance": float(
            cfg.target_normal_variance
        ),
        "targets_attack_variance": float(
            cfg.target_attack_variance
        ),
        "targets_threshold": float(
            cfg.threshold_multiplier
            * cfg.target_normal_variance
        ),
    }
...
def plot_first_n(
    df: pd.DataFrame,
    n: int = 100,
    outfile: str = "CSI_Simple_plot.png"
) -> None:
    ...
    plt.title(
        f"CSI Amplitude (first {n} samples)"
    )
...
def main() -> None:
    ...
    with open(
        ...
        f.write(
            "Synthetic CSI variance analysis\n"
        )
